...
void Voice_Send(u8 mode)
{
	 u8 LEB[8]={0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07};
		...
		if(Flag.voice==1){
			...
			USART_SendData(USART3,0xAA);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束
			USART_SendData(USART3,0x07);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束
			USART_SendData(USART3,0x02);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束
			USART_SendData(USART3,0x00);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束			
			USART_SendData(USART3,LEB[mode]);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束		
			USART_SendData(USART3,SM[mode]);         //向串口1发送数据
			while(USART_GetFlagStatus(USART3,USART_FLAG_TC)!=SET);//等待发送结束
			Flag.voice=0;
		}
}
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART2_RX_STA&0x8000)==0)//接收未完成
			...
			if(USART2_RX_STA&0x4000)//接收到了0x0d
				{
				if(Res!=0x0a){USART2_RX_STA=m;USART2_RX_STA++;USART2_RX_BUF[USART2_RX_STA&0X3FFF]=Res;}//接收错误,重新开始
				else {USART2_RX_STA=0;gyro.x_low=USART2_RX_BUF[37];gyro.x_high=USART2_RX_BUF[38];gyro.y_low=USART2_RX_BUF[39];gyro.y_high=USART2_RX_BUF[40];gyro.z_low=USART2_RX_BUF[41];gyro.z_high=USART2_RX_BUF[42];m=0;}	//接收完成了 
				}
			else //还没收到0X0D
				{	
